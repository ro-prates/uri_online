WITH menos AS (
    SELECT 
        ...
        COUNT(empregado.lotacao_div) AS pessoas 
    FROM divisao 
...
, pessoas AS(
    SELECT 
        lotacao_div, 
        COUNT(lotacao_div) AS quantidade_pessoas 
    FROM empregado
...
, final AS (
    SELECT 
        ...
        MAX(mediasDEPARTAMENTOS.media) AS media 
    FROM divisao 
...
SELECT 
    ...
    media 
FROM final fin
    GROUP BY departamento, divisao, media
    HAVING media = (SELECT MAX(media) FROM final WHERE departamento = fin.DEPARTAMENTO)
ORDER BY media desc